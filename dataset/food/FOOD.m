...
classdef FOOD < dataset
   ...
   methods
        function obj = FOOD(nTr , nTe, outputFormat , shuffleTraining, shuffleTest, shuffleAll, varargin)
...
%            Optional parameters: classes, trainClassFreq, testClassFreq            
            ...
            data = load('food_20.mat');
            
            gnd = [];% store labels 
            % change this to load X with Train Data
              obj.X = [ obj.X ; data.('train')(:,1:end-1);];
              gnd =  [gnd ; data.('train')(:,end)];

            obj.nTrTot = size(obj.X,1);%store no of total train 

            obj.X = [ obj.X ; data.('test')(:,1:end-1);];
            gnd =  [gnd ; data.('test')(:,end)];
